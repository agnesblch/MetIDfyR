...
doChromato = function(chrom_dataframe, line_size = 0.5, alpha = 0.4){
  ...
  gpl = ggplot(chrom_dataframe, aes(x=rt/60, y=intensity, alpha=alpha,
    ...
    theme_classic() +
    guides(alpha = "none") +  #don't show legend for alpha
...
doMassSpectrum = function(data_ms, polarity = NULL){ 
  ...
  ggp = ggplot(data=abund_exp, aes(mz, perctot)) +
    ...
    theme_classic() + guides(alpha = "none", colour  = "none", size="none") +
    geom_label_repel(
  ...
  if(length(data_ms2) > 0){
    if(exists_ref){
      ...
      ggplot(data_ms2, aes(mz, intensity, color = type)) +
        ...
        geom_label_repel(
          aes(mz, intensity, label = ifelse(type=="exp"|type=="precursor_exp", round(mz, 5), "")),
          size = 3,
          point.padding = unit(0.1, "lines"),
          box.padding = unit(1, "lines"),
          segment.color = 'grey50'
      ...
      ggplot(as.data.frame(cbind(data_ms2@mz, data_ms2@intensity)), aes(data_ms2@mz, data_ms2@intensity)) +
        theme_classic() + theme(axis.text = element_text(size=8,colour="black"), 
                                axis.title = element_text(size = 8), 
                                plot.title = element_text(size = 9),
                                axis.ticks=element_line(colour="black"), 
                                legend.position = "none") +
        ...
        geom_label_repel(
          aes(data_ms2@mz, data_ms2@intensity, label = round(data_ms2@mz, 5)),
          size = 3,
          point.padding = unit(0.1, "lines"),
          box.padding = unit(1, "lines"),
          segment.color = 'grey50'
